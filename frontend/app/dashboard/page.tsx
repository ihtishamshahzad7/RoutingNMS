    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 px-6 py-4">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-xs text-slate-300"><span className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${connected ? "bg-emerald-400" : "bg-amber-400"}`} />{connected ? "Backend connected" : "Backend pending"}</span><button onClick={logout} className="rounded-md border border-slate-700 px-3 py-2 hover:bg-slate-800">Logout</button></div>
        </div>
      </header>
      <section className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[["Network Health", `${networkHealth}%`, states.length ? `${running}/${states.length} OLTs running` : "No OLTs registered"],["OLT Pollers", String(running), `${totalPolls} polls completed`],["Critical Alerts", String(critical), critical ? "Immediate attention required" : "No active critical alerts"],["Open Alerts", String(alerts.length), alerts.length ? "Active incidents" : "No active incidents"]].map(([label,value,detail]) => <article key={label} className="rounded-xl border border-slate-800 bg-slate-900 p-5"><div className="text-sm text-slate-400">{label}</div><div className="mt-2 text-3xl font-semibold">{value}</div><div className="mt-2 text-xs text-slate-500">{detail}</div></article>)}
        </div>
