...
export default function TopologyPage(){
 ...
 useEffect(()=>{let live=true; const load=async()=>{try{const r=await fetch('/api/topology',{cache:'no-store'});if(!r.ok)throw new Error('Unable to load topology');const g=await r.json();if(live){setGraph(g);setError('')}}catch(e){if(live)setError(e instanceof Error?e.message:'Topology unavailable')}};load();const t=setInterval(load,10000);return()=>{live=false;clearInterval(t)}},[])
