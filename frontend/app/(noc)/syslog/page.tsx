...
function severityBadge(sev?: number) {
  if (sev === undefined) return "bg-[#21262D] text-[#8B949E]";
  if (sev <= 2) return "bg-[#2D1212] text-[#F78166]";
  if (sev === 3) return "bg-[#2D2000] text-[#D29922]";
  if (sev <= 5) return "bg-[#11233F] text-[#58A6FF]";
    ...
    <main className="mx-auto max-w-7xl px-6 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-[#8B949E]">
          ...
          Listening on UDP/TCP <code className="rounded bg-[#161B22] px-1.5 py-0.5">1514</code> by default
          (set <code className="rounded bg-[#161B22] px-1.5 py-0.5">SYSLOG_ADDR=:514</code> to use the standard port).
        </p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <select value={hostFilter} onChange={e => setHostFilter(e.target.value)} className="rounded-[6px] border border-[#30363D] bg-[#0D1117] px-3 py-2 text-sm">
          <option value="">All sources</option>
          {hosts.map(h => <option key={h} value={h}>{h}</option>)}
        </select>
        <select value={maxSeverity} onChange={e => setMaxSeverity(e.target.value)} className="rounded-[6px] border border-[#30363D] bg-[#0D1117] px-3 py-2 text-sm">
          <option value="">All severities</option>
          {Object.entries(SEVERITY_LABELS).map(([v, label]) => <option key={v} value={v}>{label} and worse</option>)}
        </select>
        <button onClick={load} className="rounded border border-[#30363D] px-3 py-2 text-xs hover:bg-[#21262D]">Refresh</button>
      </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={i.id} className="border-b border-[#21262D] align-top">
                  <td className="whitespace-nowrap px-3 py-3 text-xs text-[#8B949E]">{new Date(i.receivedAt).toLocaleString()}</td>
                  ...
                  <td className="px-3 py-3 text-xs text-[#8B949E]">{i.tag || "—"}</td>
                  <td className="px-3 py-3 text-[#E6EDF3]">{i.message}</td>
                </tr>
              )) : (
                <tr><td colSpan={5} className="px-3 py-8 text-center text-[#8B949E]">No syslog messages received yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
