...
func (h HealthHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
    ...
    if org == "" { http.Error(w, "organizationId is required", http.StatusBadRequest); return }
    devices, err := h.Repo.List(r.Context(), org)
    if err != nil { http.Error(w, "failed to load devices: "+err.Error(), http.StatusInternalServerError); return }
    ...
    if d.SNMPEnabled {
        ...
        if err == nil {
            ...
            if connectErr == nil {
                ...
                _ = client.Conn.Close()
            } else { result.Error = connectErr.Error() }
        } else { result.Error = err.Error() }
    } else {
        result.Method = "TCP"
