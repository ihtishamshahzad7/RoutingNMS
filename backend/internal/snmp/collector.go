...
func (Collector) Get(ctx context.Context,target Target,oids []string)([]Value,error){
	if len(oids)==0{return nil,fmt.Errorf("SNMP OID list is empty")}; client,err:=(Collector{}).Connect(ctx,target);if err!=nil{return nil,err};defer client.Conn.Close();if err:=ctx.Err();err!=nil{return nil,err};packet,err:=client.Get(oids);if err!=nil{return nil,fmt.Errorf("snmp get %s: %w",target.Address,err)};return packetValues(packet),nil
}
func (Collector) Walk(ctx context.Context,target Target,oid string)([]Value,error){
	if oid==""{return nil,fmt.Errorf("SNMP walk OID is required")}; client,err:=(Collector{}).Connect(ctx,target);if err!=nil{return nil,err};defer client.Conn.Close();values:=make([]Value,0);err=client.Walk(oid,func(pdu gosnmp.SnmpPDU)error{if err:=ctx.Err();err!=nil{return err};values=append(values,Value{OID:pdu.Name,Value:pdu.Value,Timestamp:time.Now().UTC()});return nil});if err!=nil{return nil,fmt.Errorf("snmp walk %s %s: %w",target.Address,oid,err)};return values,nil
}
