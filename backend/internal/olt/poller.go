...
func (p Poller) poll(ctx context.Context) error {
	...
	for _,port:=range ports{
		...
		polled:=make([]ONU,0,len(onus));for _,onu:=range onus{updated,err:=p.Adapter.PollONU(ctx,p.OLT,onu);if err!=nil{if ctx.Err()!=nil{return ctx.Err()};log.Printf("ONU poll failed olt=%s onu=%s: %v",p.OLT.ID,onu.ID,err);polled=append(polled,onu);continue};polled=append(polled,updated);for _,a:=range EvaluateONU(updated,policy){alerts=append(alerts,a);active[updated.ID+":"+a.Code]=true;if p.Repo.DB!=nil{if err:=p.Repo.UpsertAlert(ctx,p.OLT.ID,port.ID,updated.ID,a,result.PolledAt);err!=nil{return fmt.Errorf("persist alert: %w",err)}}}}
		port.ONUs=polled;port.ONUCount=len(polled);result.PONs=append(result.PONs,port);result.ONUs=append(result.ONUs,polled...)
	}
	if p.Repo.DB!=nil {if err:=p.Repo.ClearResolvedAlerts(ctx,p.OLT.ID,active,result.PolledAt);err!=nil{return fmt.Errorf("clear resolved alerts: %w",err)};if err:=p.Repo.SavePollResult(ctx,p.OLT.ID,result);err!=nil{return fmt.Errorf("persist poll result: %w",err)}}
