...
// ConfigService loads enabled OLTs and their complete SNMP configuration.
type ConfigService struct { DB *pgxpool.Pool; Profiles *ProfileRegistry }

type ConfiguredOLT struct { OLT OLT; SNMP snmp.Target; Profile VendorProfile }
...
func (s ConfigService) LoadEnabled(ctx context.Context) ([]ConfiguredOLT,error) {
	...
	for rows.Next(){
		...
		t:=snmp.Target{Address:o.Address,Port:161,Timeout:snmp.DefaultTimeout,Retries:snmp.DefaultRetries,Credentials:snmp.Credentials{Version:snmp.Version(version),Community:community,Username:username,AuthProto:authProto,AuthPass:authPass,PrivProto:privProto,PrivPass:privPass}}
		out=append(out,ConfiguredOLT{OLT:o,SNMP:t,Profile:profile})
	}
