...
func (a *SNMPAdapter) PollONU(ctx context.Context, olt OLT, onu ONU)(ONU,error){
	...
	for _,m:=range metrics{if !m.template.Valid(){continue};oid,err:=m.template.Indexed(parts...);if err!=nil{return onu,err};oids=append(oids,oid);active=append(active,m)}
