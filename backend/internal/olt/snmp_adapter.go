...
func (a *SNMPAdapter) DiscoverONUs(ctx context.Context, olt OLT, port PONPort) ([]ONU,error) {
	...
	onus:=make([]ONU,0,len(values))
	for _,v:=range values { onus=append(onus,ONU{ID:v.OID,OLTID:olt.ID,PONPortID:port.ID,Serial:strings.TrimSpace(fmt.Sprint(v.Value)),Status:Unknown}) }
	return onus,nil
}
...
func (a *SNMPAdapter) PollONU(ctx context.Context, olt OLT, onu ONU)(ONU,error){
	...
	metrics:=[]metric{
		{a.Mapping.ONUStatusOID,func(v snmp.Value){onu.Status=parseStatus(v.Value)}},
		{a.Mapping.ONULOSOID,func(v snmp.Value){onu.LOS=parseBool(v.Value)}},
		{a.Mapping.ONURXPowerOID,func(v snmp.Value){if n,ok:=parseFloat(v.Value);ok{onu.RxPowerDBm=&n}}},
		{a.Mapping.ONUTXPowerOID,func(v snmp.Value){if n,ok:=parseFloat(v.Value);ok{onu.TxPowerDBm=&n}}},
	}
	...
	values,err:=a.pollIndexed(ctx,olt,onu,oids);if err!=nil{return onu,err}
	for i,v:=range values{if i<len(active){active[i].set(v)}}
