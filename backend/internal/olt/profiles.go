...
func (s ONUIndexSpec) Extract(index string)(string,string,error){ if !s.Valid(){return "","",fmt.Errorf("ONU index specification is not configured")}; parts:=strings.Split(strings.Trim(index,"."),"."); pick:=func(pos []int)(string,error){out:=make([]string,0,len(pos));for _,p:=range pos{if p<0||p>=len(parts){return "",fmt.Errorf("ONU index position %d is outside index %q",p,index)};out=append(out,parts[p])};return strings.Join(out,s.Separator),nil}; pon,err:=pick(s.PONPositions);if err!=nil{return "","",err};onu,err:=pick(s.ONUPositions);if err!=nil{return "","",err};return pon,onu,nil }
...
// normalizedMapping establishes one authoritative runtime mapping. Legacy profile fields are copied only when Mapping is incomplete.
...
func (r *ProfileRegistry) Match(vendor,model string)(VendorProfile,bool){vendor,model=strings.ToLower(strings.TrimSpace(vendor)),strings.ToLower(strings.TrimSpace(model));for _,p:=range r.profiles{if strings.ToLower(strings.TrimSpace(p.Vendor))!=vendor{continue};if len(p.Models)==0{p.Mapping=p.normalizedMapping();return p,true};for _,m:=range p.Models{if strings.EqualFold(strings.TrimSpace(m),model){p.Mapping=p.normalizedMapping();return p,true}}};return VendorProfile{},false}
